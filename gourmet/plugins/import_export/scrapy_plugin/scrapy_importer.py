...
class ScrapyImporter(ConvenientImporter, NotThreadSafe):
    ...
    def do_run (self):
        ...
        if 'imageUrl' in recipe:
            ...
            print("Load image from url: %s" %(imageUrl))
            r = requests.get(imageUrl)
            # Using startswith because some servers send an invalid response like
            # image/jpeg;charset=UTF-8             
            if r.status_code == 200 and r.headers['content-type'].startswith('image/jpeg'):
                recipe["image"] = r.content
            else:
                print("Problem while loading image from url \"%s\": status-code: %i, "
                      "content-type: %s" %(imageUrl,r.status_code,r.headers['content-type']))
